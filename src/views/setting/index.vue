<template>
  <div class="dashboard-container">
    ...
    <el-dialog title="分配权限" :visible="showPermDialog" @close="btnPermCancel">
      <!-- 权限是一棵树 -->
      <!-- 将数据绑定到组件上 -->
      <!--  :check-strictly 如果为true 那表示父子勾选时不互相关联 如果为false就不互相关联 -->
      <!-- id作为唯一的标识 -->
      <el-tree
        ref="permTree"
        :data="permData"
...
export default {
  ...
  methods: {
    ...
    async getCompanyInfo() {
      this.formData = await getCompanyInfo(this.companyId) // 赋值给formData承载公司信息
    },
    changePage(newPage) {
      // newPage是当前点击的页码
      // 将当前页码赋值给当前的最新页码
    ...
    async assignPerm(id) {
      ...
      this.roleId = id
      // 应该去获取这个id的权限点
      const { permIds } = await getRoleDetail(id) // permId是当前角色所拥有的权限点数据
